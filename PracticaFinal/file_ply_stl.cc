...
// constantes y funciones auxiliares:
  ...
  void leer_caras (unsigned num_vertices, unsigned num_caras,
    ...
    void leer_vertices_y_texturas(unsigned num_vertices, 
                               vector<float>& vertices, 
                               vector<float>& texturas,  // Añadimos el vector de texturas
                               ifstream& src);
...
// funcion principal de lectura
  ...
  void read
    (const char *nombre_archivo_pse,
     vector < float >&vertices, vector < int >&caras,vector<float>& texturas  )
  {
    // modelos ply en:
    ...
    if (na.substr (na.find_last_of (".") + 1) != "ply")
      ...
      leer_vertices_y_texturas(num_vertices, vertices, texturas, src);
                               ...
                               ifstream& src)
{
    ...
    // Redimensionar el vector de vértices y texturas
    vertices.resize(num_vertices * 3);   // Vértices: 3 componentes por vértice (x, y, z)
    texturas.resize(num_vertices * 2);    // Texturas: 2 componentes por vértice (s, t)
    ...
    for (long long iv = 0; iv < num_vertices; iv++) {
        ...
        double x, y, z, s = 0.0, t = 0.0;

        // Leer las coordenadas del vértice (x, y, z)
        src >> x >> y >> z;

        // Leer las coordenadas de textura (s, t) si están disponibles
        if (src >> s >> t) {
            // Asumimos que siempre hay una propiedad s y t si el archivo tiene coordenadas de textura
            texturas[iv * 2] = s;
            texturas[iv * 2 + 1] = t;
        }

        // Ignorar cualquier propiedad extra que pueda haber (por ejemplo, colores)
        src.getline(buffer, tam_buffer); 

        // Agregar el vértice al vector de vértices
        long long base = iv * 3;
        vertices[base + 0] = x;
        vertices[base + 1] = y;
        vertices[base + 2] = z;
