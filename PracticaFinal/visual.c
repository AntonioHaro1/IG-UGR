...
**/
void setCamara (float ax, float ay, float az, float d, float Iz, float De)
{
  view_rotx = ax;
  view_roty = ay;
  view_rotz = az;
...
void getCamara (float &ax, float &ay, float &az, float &d, float &Iz,float &De)
{
  ax = view_rotx;
  ay = view_roty;
  az = view_rotz;
...
void get_angulo_xz(float &ay, float &Iz,float &De){
  ay = view_roty;
  Iz = Izq;
  De = Der;
}

void setCamaraTecla (float Iz, float De)
{
  //Limitamos el movimiento sobre el mapa
  if(Iz > 3 && Iz < 118 ){
    Izq = Iz; 
  }
  if(De > -21 && De < 21)
